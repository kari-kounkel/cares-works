...
const BUILD_PROGRESS = [
  { label: "Purchase Orders", items: [
    ...
    ["Convert PO → invoice (confirmed, never lost)", "done"],
    ["Artwork / proof on the order", "done"],
    ["One Orders page — current + closed", "done"],
  ] },
  { label: "Invoicing", items: [
    ...
    ["Payment terms / due date", "done"],
    ["Invoice & PO numbers can't collide", "done"],
  ] },
  { label: "Bank & card setup", items: [
    ...
    ["Plaid — pull new transactions in", "done"],
    ["Bank feed auto-syncs + de-dupes", "done"],
  ...
  { label: "Notebook", items: [
    ...
    ["Plain register view (your own)", "done"],
    ["Sync the bank feed right here", "done"],
  ...
  { label: "History — copy from past work", items: [
    ...
    ["Invoice history imported — 5,755 invoices, 1,309 customers", "done"],
    ["Copy an old line item into a new PO / new customer", "todo"],
    ["Line-item detail on old invoices (headers imported)", "todo"],
  ...
  function Orders() {
    ...
    return (
      <div>
        ...
        <div style={{ fontSize: 12, color: N.muted, marginTop: 10 }}>An order holds the full spec so it can be reordered. Send the PO to your vendor; when it's made, hit <b style={{ color: N.blue }}>Convert to invoice</b> to bill the customer. Closed orders — already billed — sit under the divider.</div>
      </div>
    );
  }
...
{isCust && c.tax_status === "Shipped" && <span style={{ fontSize: 10, fontWeight: 700, color: N.muted, marginLeft: 6, letterSpacing: "0.04em" }}>SHIPPED</span>}{isCust && (() => { const cc = (entity.credits || []).filter(cr => cr.status === "open" && (cr.customer_name || "").toLowerCase() === (c.name || "").toLowerCase()).reduce((s, cr) => s + (cr.amount_cents || 0), 0); return cc > 0 ? <span style={{ fontSize: 10, fontWeight: 700, color: N.pinkDark, background: "#eafaf0", border: "1px solid #bff0d3", borderRadius: 100, padding: "2px 8px", marginLeft: 8 }}>{money(cc / 100)} CREDIT</span> : null; })()}</div>
  ...
  else if (activeSection === "purchaseorders") body = Orders();
