...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let to = String(inv.customer_email || "").trim();
    if (!to) {
      ...
      to = String(cust?.email || "").trim();
    }
    if (!to) return json({ error: "This customer has no email on file." }, 400);
    ...
    const subject = "Payment received — invoice" + num + " paid in full";
    ...
    const html = '<div style="font-family:Arial,Helvetica,sans-serif;font-size:15px;color:#0f172a;line-height:1.6">' +
      ...
      "<p>This confirms we’ve received your payment for invoice" + esc(num) + " — <b>" + money(inv.total_cents) + "</b>, <b>paid in full</b>. Thank you!</p>" +
      '<p style="display:inline-block;background:#eafaf0;border:1px solid #bff0d3;color:#1b7a45;border-radius:8px;padding:10px 16px;font-weight:700">PAID IN FULL — ' + money(inv.total_cents) + '</p>' +
      "<p>We appreciate your business.</p>" +
      "<p>" + esc(orgName) + "</p>" +
      '<p style="color:#94a3b8;font-size:12px">Sent securely via CARES Works.</p>' +
      "</div>";
    const text = "Hi " + (inv.customer_name || "there") + ",\n\nThis confirms we've received your payment for invoice" + num + " — " + money(inv.total_cents) + ", paid in full. Thank you!\n\n" + orgName + "\n\nSent securely via CARES Works.";
    ...
    const hubRes = await fetch(hubUrl, {
      ...
      body: JSON.stringify({ action: "notify", channel: "email", type: "receipt", source: "cares-works", to, subject, body: text, html, from_name: orgName, ...(fromEmail ? { from_email: fromEmail } : {}), ...(replyTo ? { reply_to: replyTo } : {}), ...(senderEmail ? { bcc: senderEmail } : {}) }),
    });
