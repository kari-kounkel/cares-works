...
- Other books/workbooks awaiting their own subdomain (Kari's list, 8/29): mustard; crash / sorrow (bus crash — one book + one workbook); pickled pardon; the coming / countdown / chasing chickens; culture under construction; qb unmasked. Only `ladybug` is live. **Do not create these DNS records ahead of the pages** — a CNAME pointing at Vercel with no project claiming it serves a Vercel error page, which is worse than not resolving. Add each one the day its page ships.
