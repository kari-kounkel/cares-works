...
**Pending / frozen items**
- ✅ RESOLVED 9/6: `accounts.karikounkel.com` is LIVE. Repo `kari-kounkel/accounts` (Kari created it — this remote session can create neither GitHub repos nor Vercel projects, both 403), Vercel project `accounts` imported from it, DreamHost CNAME `accounts` → `cname.vercel-dns.com` confirmed from ns1.dreamhost.com, Cloudflare and Google. The site is her real July `kkstore/coa` pages, moved over: the book page plus `tools.html`, which `cleanUrls` serves at `/tools` — the printed barcode address. Root `karikounkel.com` MX (Google) and A (67.205.2.111) verified untouched throughout.
- Court of Accounts order buttons still carry placeholder hrefs (`STRIPE_LINK_DIGITAL` / `_DELUXE` / `_PRINT`). A guard in `index.html` turns any unset button into a pre-filled email order so nobody 404s, but real Stripe Payment Links are still needed — Kari has to supply them or authorize the Stripe connector (this session cannot; it needs OAuth). Also still needed for paid delivery: `STRIPE_SECRET_KEY` + `SUPABASE_SERVICE_ROLE_KEY` on the Vercel project, and a private `coa-pdf` bucket in the kkstore Supabase holding `court-of-accounts.pdf`.
- Old pending item, now historical: Two steps, both Kari's: add the domain to the `cares-works` Vercel project, and add the CNAME at DreamHost. The pages it should land on are built and live in the meantime at `tools.caresmn.com/tools` and `tools.caresmn.com/court-of-accounts`.
